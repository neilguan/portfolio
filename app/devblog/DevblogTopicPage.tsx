...
function Artifact({ item }: { item: DevblogArtifact }) {
  return <figure className="engineering-artifact">
    <div className="engineering-media">
      {item.kind === 'audio' ? <audio controls preload="metadata" src={item.src} aria-label={item.label} />
        : item.kind === 'video' ? <video controls preload="metadata" playsInline src={item.src} aria-label={item.label} />
        : <a href={item.src} target="_blank" rel="noreferrer" aria-label={`Open ${item.label}`}><img src={item.src} alt={item.label} loading="lazy" /></a>}
    </div>
...
export function BlogHeader() {
  return <header className="site-header engineering-header"><a className="wordmark" href="/">Neil Guan</a><nav aria-label="Main navigation"><a href="/devblog">Development log</a><a href="/">Portfolio</a></nav></header>;
}
...
export default function DevblogTopicPage({ slug, topic }: { slug: string; topic: DevblogTopic }) {
  return <main className="engineering-blog" data-topic={slug}>
    ...
    <header className="engineering-title">
      <a href="/devblog" className="engineering-back">← All development notes</a>
    ...
    <div className="engineering-layout">
      ...
      <article>
        ...
        {topic.steps.map((s,index)=><section id={s.id} className="engineering-section" key={s.id}>
          ...
          {s.sources.length>0 && <p className="engineering-sources"><span>Source records</span>{s.sources.map(s=><a key={s.href} href={s.href} target="_blank" rel="noreferrer">{s.label}</a>)}</p>}
        </section>)}
        ...
        <footer className="engineering-article-footer"><a href="/devblog">Back to development log</a></footer>
      </article>
    </div>
  </main>;
}
